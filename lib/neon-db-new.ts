...
export async function updateUser(
  ...
  if (password) {
    await sql`
      UPDATE users 
      SET nik = ${nik}, nama = ${nama}, password = ${password}, role = ${role}
      WHERE id = ${id}
    `
  } else {
    await sql`
      UPDATE users 
      SET nik = ${nik}, nama = ${nama}, role = ${role}
      WHERE id = ${id}
    `
  }
}
...
export async function addLeaveRequest(
  ...
  const result = await sql`
    INSERT INTO leave_requests (
      ...
      ${data.userId}, ${data.userName}, ${data.userNik}, ${data.userSite},
      ${data.userDepartemen}, ${data.userJabatan}, ${data.jenisIzin},
      ${data.tanggalMulai}, ${data.tanggalSelesai}, ${data.jumlahHari},
      ${data.keterangan}, ${data.status}, ${data.approvalLevel}
    )
    RETURNING *
  `
  const row = result[0]
...
export async function updateLeaveRequest(id: number, updates: Partial<LeaveRequest>): Promise<LeaveRequest> {
  const sql = getSql()
  const result = await sql`
...
export async function getApprovalsByRequestId(requestId: string): Promise<Approval[]> {
  const sql = getSql()
  const result = await sql`SELECT * FROM approvals WHERE leave_request_id = ${requestId} ORDER BY level ASC`
  return result.map((row: any) => ({
    id: row.id,
    leaveRequestId: row.leave_request_id,
    approverId: row.approver_id,
...
export async function getApprovalsByApproverId(approverId: string): Promise<Approval[]> {
  const sql = getSql()
  const result = await sql`SELECT * FROM approvals WHERE approver_id = ${approverId} ORDER BY created_at DESC`
  return result.map((row: any) => ({
    id: row.id,
    leaveRequestId: row.leave_request_id,
    approverId: row.approver_id,
...
export async function addApproval(data: Omit<Approval, "id" | "createdAt" | "updatedAt">): Promise<Approval> {
  const sql = getSql()
  const result = await sql`
    INSERT INTO approvals (
      ...
      ${data.leaveRequestId}, ${data.approverId}, ${data.approverName},
      ${data.approverNik}, ${data.approverJabatan}, ${data.level},
...
export async function updateApproval(id: number, updates: Partial<Approval>): Promise<Approval> {
  const sql = getSql()
  const result = await sql`
    UPDATE approvals 
    SET 
