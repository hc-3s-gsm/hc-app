...
export async function getLeaveRequestsByUserId(userId: number): Promise<LeaveRequest[]> {
  const sql = getSql()
...
export async function getApprovalsByRequestId(leaveRequestId: number): Promise<Approval[]> {
  const sql = getSql()
  const result = await sql`SELECT * FROM approvals WHERE leave_request_id = ${leaveRequestId} ORDER BY level ASC`
  return result.map((row: any) => ({
    id: row.id,
    leaveRequestId: row.leave_request_id,
    approverId: row.approver_id,
...
export async function getApprovalsByApproverId(approverId: number): Promise<Approval[]> {
  const sql = getSql()
